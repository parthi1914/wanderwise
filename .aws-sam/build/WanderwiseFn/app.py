...
# --------------------------------------------------------------------------- #
def _search_places(query, limit=5):
    ...
    for r in results[:limit]:
        matches.append({
            ...
            "lat": r.get("latitude"),
            "lon": r.get("longitude"),
            "label": ", ".join(part for part in [r.get("name"), r.get("admin1"), r.get("country")] if part),
        })
    return matches


def tool_geocode(name):
    results = _search_places(name, 1)
    if not results:
...
def _search_handler(event):
    ...
    if not params and event.get("multiValueQueryStringParameters"):
        params = {k: v[0] if isinstance(v, list) and v else v for k, v in event["multiValueQueryStringParameters"].items()}
    q = (params.get("q") or "").strip()
    if not q:
        return _json(400, {"ok": False, "error": "Missing 'q' query parameter."})
    return _json(200, {"ok": True, "results": _search_places(q, 5)})
...
def handler(event, context):
    ...
    if method == "GET" and raw_path.endswith("/search"):
        return _search_handler(event)
    if method == "GET":
        return _html()
    if method != "POST":
